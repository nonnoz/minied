...
import { DraftOrder, Order, PartOrder } from './data'
...
app.get("/api/customer/draft-order", checkAuthenticated, async (req, res) => {
  ...
  res.status(200).json(draftOrder || { customerId, partOrder: {
    time: new Date(),
    title: "",
    content: "",
    tag: "",
    private: false,
    // randomId: Math.ceil(Math.random()*100000).toString,
} })
})
...
client.connect().then(() => {
  ...
  Issuer.discover("http://127.0.0.1:8081/auth/realms/minied/.well-known/openid-configuration").then(issuer => {
    ...
    // start server
    app.listen(port, () => {
      logger.info(`minied server listening on port ${port}`)
    })
  })
})
